...
class SortingConsumer(WebsocketConsumer):
    def connect(self):
        async_to_sync(self.channel_layer.group_add)(
            "sorters_room", self.channel_name
        )
    ...
    def sorters_message(self, event):
        message = event["message"]
        self.send(text_data=json.dumps({"message": message}))

    def send_opened_by_others_state_to_all(self):
        async_to_sync(self.channel_layer.group_send)(
            "sorters_room", {"type": "sorters_message", "message": opened_by_others}
        )

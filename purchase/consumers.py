...
class PurchaseConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data):
        ...
        if data.pop("action") == "open":
            ...
            for opened_by in opened_by_others:
                if opened_by["product_id"] == data["product_id"]:
                    found = True
                    break
            ...
            for opened_by in opened_by_others:
                if opened_by["product_id"] == data["product_id"] and opened_by["user_id"] == user.id:
                    opened_by_others.remove(opened_by)
                    self.send_opened_by_others_state_to_all()
                    break

    def buyers_message(self, event):
        message = event["message"]
        self.send(text_data=json.dumps({"message": message}))

    def send_opened_by_others_state_to_all(self):
        async_to_sync(self.channel_layer.group_send)(
            "buyers_room", {"type": "buyers_message", "message": opened_by_others}
        )
